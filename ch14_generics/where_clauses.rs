 ...
 * rather than at the type's first mention. Additionally, where clauses can apply bounds 
 * to arbitrary types, rather than just to type parameters.
 *
 ...
 *   example cannot be directly expressed without a where clause:
 */
...
impl<T> PrintInOption for T where
    ...
    // otherwise would be using the wrong bound.
    fn print_in_option(self) {
        println!("{:?}", Some(self));
    }
}
