 ...
 * this ambiguity is not allowed when writing functions. When taking a closure as an 
 * input parameter, the closure's complete type must be annotated using one of a few 
 * traits. In order of decreasing restriction, they are:
 *
 ...
 * closure may capture by &T, &mut T, or T, but the compiler will ultimately choose 
 ...
 * then capturing variables by &mut T or T are not allowed.
 *
 * In the following example, try swapping the usage of Fn, FnMut, and FnOnce to 
 * see what happens:
 */
