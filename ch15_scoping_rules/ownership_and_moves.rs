 ...
 * owner. This also prevents resources from being freed more than once. Note that not all 
...
fn main() {
    ...
    // *Copy* 'x' into 'y' - no resources are movec.
    let y = x;
    ...
    // Error! 'a' can no longer access the data, because it no longer owns th heap memory.
    // println!("a contains: {}", a);

    // This function takes ownership of the heap allocated memory from 'b'
    destroy_box(b);

    // Since the heap memory has been freed at this point, this action would result in 
    // dereferencing freed memory, but it's forbidden by the compiler.
